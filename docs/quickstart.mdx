...
<details>
<summary><strong>Option C — OpenAI (ChatGPT / Codex subscription)</strong></summary>

```bash
atx provider add openai        # browser OAuth (PKCE)
```

Then, at project init: `atx project init --provider=openai`. Subscription
OAuth and API-key routing share the same `openai` provider identity.
</details>

More options (Bedrock, LiteLLM, Gemini, Ollama, OpenRouter, Lilac, and
CoralBricks) live in [Providers and Authentication](/providers). Each provider
guide lists its supported labs, models, and exact slugs.
...
ATX inspects the repo, generates a specialized agent fleet, registers the
project with the daemon, and wires the local integration files ATX uses
for hooks and MCP. Re-run with `--force` any time your codebase has
changed enough that you want the fleet regenerated.
...
with full logs and sequence diagrams. Details in [Dashboard](/dashboard).
